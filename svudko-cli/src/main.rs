...
use anyhow::Context;
use svudko_common::{DEFAULT_SERVER_ADDR, dummy_verification::SkipServerVerification};
use svudko_common::{
    SERVER_PORT,
    quinn::{
...
async fn main() -> anyhow::Result<()> {
    let Args { subcommand, .. } = <Args as clap::Parser>::parse();
    ...
    while let Ok(effect) = rx.recv() {
        match effect {
            svudko_core::Effect::Render(_) => (),
            svudko_core::Effect::Error(_) => panic!("error in core"),
        }
    }
